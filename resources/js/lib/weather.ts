...
function readConfig(): WeatherConfig | null {
    ...
    if (
        !import.meta.env.VITE_WEATHER_LATITUDE ||
        !import.meta.env.VITE_WEATHER_LONGITUDE
    )
        return warnAndDisable();
...
export async function fetchCurrentWeather(
    ...
    const { current } = await getJson<OpenMeteoResponse>(
        currentWeatherUrl(weatherConfig),
        { signal },
    );
