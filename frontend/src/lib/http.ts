...
export async function getJson<T>(url: string, { signal, timeoutMs = 8000 }: GetJsonOptions = {}): Promise<T> {
  ...
  const response = await fetch(url, { signal: composed, headers: { Accept: "application/json" } });
